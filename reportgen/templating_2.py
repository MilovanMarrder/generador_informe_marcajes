...
{% for r in regs -%}
{# --- COMIENZO DE LA LÓGICA DE FILA --- #}
{% if r['jornada'].total_seconds() == 0 -%}
\rowcolor{jornadacero!20}
{%- endif -%}
{%- if r['dia'] in ['Sáb', 'Dom'] -%}
{# Fila de fin de semana (todo en negrita) #}
\textbf{ {{ r['fecha'].strftime('%Y-%m-%d') }} } & \textbf{ {{ r['dia'] }} } & \textbf{ {{ r['entrada'].time() }} } & \textbf{ {{ r['salida'].time() }} } &
    {%- if r['jornada'].total_seconds() == 0 -%}
        ...
        \textbf{ \textcolor{jornadacero}{ {{ "%.2f"|format(r['jornada'].total_seconds()/3600) }} } }
    {%- else -%}
...
{{ r['fecha'].strftime('%Y-%m-%d') }} & {{ r['dia'] }} & {{ r['entrada'].time() }} & {{ r['salida'].time() }} &
    {%- if r['jornada'].total_seconds() == 0 -%}
        {# Jornada cero en día de semana: solo texto rojo #}
        \textcolor{jornadacero}{ {{ "%.2f"|format(r['jornada'].total_seconds()/3600) }} }
    {%- else -%}
        {# Jornada normal en día de semana: sin formato especial #}
        {{ "%.2f"|format(r['jornada'].total_seconds()/3600) }}
    {%- endif %}
{%- endif -%}
{# --- FIN DE LA LÓGICA DE FILA --- #}
\\
{% endfor %}
\bottomrule
\end{tabular}
